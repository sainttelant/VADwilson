...
if __name__ == '__main__':
    ...
    print("sample_token_list's length is ", len(sample_token_list))
    nusc = NuScenes(version='v1.0-trainval', dataroot='./data/nuscenes', verbose=True)
    numofinit = 500
    numsofvisualization = 1000
    countofvisualization = 0
    imgs = []
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    video_path = osp.join(out_path, 'vis_prune.mp4')
    ...
    for id in tqdm(range(len(sample_token_list))):
        ...
        countofvisualization += 1
        if countofvisualization <= numofinit:
            continue
            
        if countofvisualization > numsofvisualization:
            print("visualization finished,and visualized {} samples", countofvisualization)
            break
